...
public interface VotoPropuestaRepository extends JpaRepository<Votopropuesta, Integer>{   
   ...
   // Métodos para obtener los nombres de los votantes
  ...
  @Query("SELECT NEW com.boot.DTO.PropuestasListDTO(" + // <-- CLAVE: Apunta a tu DTO
         "    p.id, " +
		  "    pr.usuario.nombre, " +
         "    p.fecha " + 
//         "    SUM(CASE WHEN vp.isAFavor = 1 THEN 1 ELSE 0 END), " + // Suma 1 si es a favor, 0 si no => COMENTADO POR PROBLEMA CON EL DTO, QUE FUE MODIFICADO Y ESTA QUERY QUEDÓ OBSOLETA
//         "    SUM(CASE WHEN vp.isAFavor = 0 THEN 1 ELSE 0 END)" + // Suma 1 si es en contra, 0 si no
  ...
  List<PropuestasListDTO> findPropuestasWithVoteSummaryByActiveYearAndCasaId(@Param("idCasa") int idCasa);
	...
	Optional<Votopropuesta> findByPropuestaIdAndParticipanteromeriaId(int idPropuesta, int idParticipanteromeria);
